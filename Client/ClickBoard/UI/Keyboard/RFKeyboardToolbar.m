...
- (void)layoutSubviews {
    ...
    /*CGFloat width = _toolbarView.frame.size.width;
    CGFloat buttonWidths = 0;
    for (RFToolbarButton *button in self.buttons) {
        buttonWidths += button.frame.size.width + 8;
    }
    
    CGFloat difference = width - buttonWidths;
    NSLog(@"%f", width);
    NSLog(@"%f", buttonWidths);
    NSLog(@"%f", difference);
    _scrollView.frame = CGRectOffset(_toolbarView.frame, difference / 2, 0);*/
}
